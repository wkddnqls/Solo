          ...
          <table>
            ...
            <tbody>
              ...
              <c:if test="${empty humanList}">
                <tr><td colspan="5" class="empty-message">등록된 유저가 없습니다.</td></tr>
              </c:if>
            </tbody>
          </table>
        </div>
        ...
        <c:if test="${not empty humanList and Humanpi.maxPage > 1}">
          <div class="pagination">
            <c:if test="${Humanpi.currentPage > 1}">
              <a href="${pageContext.request.contextPath}/humanList.ep?humanPage=${Humanpi.currentPage - 1}">
                ← 이전
              </a>
            </c:if>
            <c:if test="${Humanpi.currentPage == 1}">
            ...
            <c:forEach var="i" begin="${Humanpi.startPage}" end="${Humanpi.endPage}">
              <c:choose>
                <c:when test="${i == Humanpi.currentPage}">
                  <button class="active" disabled>${i}</button>
                </c:when>
                <c:otherwise>
                  <a href="${pageContext.request.contextPath}/humanList.ep?humanPage=${i}">
                    ${i}
                  </a>
                </c:otherwise>
              </c:choose>
